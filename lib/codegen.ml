...
let rec codegen_expr = function
  ...
  | Syntax.Variable name -> (
      ...
      with Not_found ->
        raise (Error (Printf.sprintf "unknown variable name: %s" name)))
        ...
        | Some f ->
            ...
            if Array.length (params f) == List.length args then ()
              ...
              raise
                (Error
                   (Printf.sprintf
                      "redefinition of function %s with mismatched args" name));
            f
      in
      let ps = params f in
      (* set argument names *)
      ...
      try
        let ret_val =
          ...
          | _ ->
              raise
                (Error
                   "functions only support single return statements (for now!)")
        in
